...
namespace CoinPayment
{
    public class CoinApi
    {
        ...
        private  async Task<HttpResponseMessage> SendRequestAsync(BaseTransaction transaction)
        {
            ...
            return (HttpResponseMessage)await Url.WithHeaders(new Dictionary<string, string> { ["HMAC"] = hmac, ["Content-Type"] = "application/x-www-form-urlencoded" }).PostAsync(new CapturedUrlEncodedContent(query));
        }
